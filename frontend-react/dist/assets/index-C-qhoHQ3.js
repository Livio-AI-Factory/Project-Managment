...
<div class="footer">Livio Building Systems &nbsp;·&nbsp; ${e.name} &nbsp;·&nbsp; Generated ${new Date().toLocaleDateString()}</div>
  ...
  </div>`}async function yo(e){const t=Ji(),n={to:e.to,cc:e.cc||"",subject:e.subject,message:e.message,fromName:e.fromName||xx()};n.replyTo=e.replyTo||hx(),Array.isArray(e.attachments)&&e.attachments.length&&(n.attachments=e.attachments),e.generatedPdf&&(n.generatedPdf=e.generatedPdf);const o=await fetch(t+"/email/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)}),r=await o.json().catch(()=>({}));if(!o.ok)throw new Error(r.error||r.message||"SMTP send failed ("+o.status+")");return r}async function yx(e){const t=Ji(),n=await fetch(t+"/email/verify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({})}),o=await n.json().catch(()=>({}));if(!n.ok||o.ok===!1)throw new Error(o.error||o.message||"SMTP verify failed ("+n.status+")");return o}function vf(){const e=_t(),t=h("cfg-api-base");t&&(t.value=Ji());const n=h("cfg-smtp-host");n&&(n.value=e.host||"");const o=h("cfg-smtp-port");o&&(o.value=e.port||"587");const r=h("cfg-smtp-user");r&&(r.value=e.user||"");const i=h("cfg-smtp-pass");i&&(i.value=e.pass||"");const a=h("cfg-smtp-secure");a&&(a.checked=!!e.secure);const c=h("cfg-from-name");c&&(c.value=e.fromName||"Livio Building Systems");const l=h("cfg-from-email");l&&(l.value=e.fromEmail||""),Hl()}function Hl(){const e=h("email-cfg-status");e&&(e.innerHTML=Ln(_t()))}function bx(){var n,o,r,i,a,c,l,d;const e=(((n=h("cfg-api-base"))==null?void 0:n.value)||"").trim(),t={apiBase:(dx(e)?dr:e)||dr,host:(((o=h("cfg-smtp-host"))==null?void 0:o.value)||"").trim(),port:(((r=h("cfg-smtp-port"))==null?void 0:r.value)||"").trim()||"587",user:(((i=h("cfg-smtp-user"))==null?void 0:i.value)||"").trim(),pass:(((a=h("cfg-smtp-pass"))==null?void 0:a.value)||"").trim(),secure:!!((c=h("cfg-smtp-secure"))!=null&&c.checked),fromName:(((l=h("cfg-from-name"))==null?void 0:l.value)||"").trim()||"Livio Building Systems",fromEmail:(((d=h("cfg-from-email"))==null?void 0:d.value)||"").trim()};if((t.host||t.user||t.pass)&&(!t.host||!t.port||!t.user||!t.pass)){j("⚠ Fill all SMTP fields or leave them all blank to use backend defaults");return}localStorage.setItem("livio_email_config",JSON.stringify(t)),Hl(),j("✅ SMTP settings saved")}function wx(){confirm("Clear saved SMTP settings? The app will then rely on backend/.env only.")&&(localStorage.removeItem("livio_email_config"),h("cfg-api-base")&&(h("cfg-api-base").value=dr),h("cfg-smtp-host")&&(h("cfg-smtp-host").value=""),h("cfg-smtp-port")&&(h("cfg-smtp-port").value="587"),h("cfg-smtp-user")&&(h("cfg-smtp-user").value=""),h("cfg-smtp-pass")&&(h("cfg-smtp-pass").value=""),h("cfg-smtp-secure")&&(h("cfg-smtp-secure").checked=!1),h("cfg-from-name")&&(h("cfg-from-name").value="Livio Building Systems"),h("cfg-from-email")&&(h("cfg-from-email").value=""),Hl(),j("🗑 SMTP settings cleared"))}async function Cx(){const e=prompt("Enter your email address to receive a test email:","");if(!e||!e.includes("@")){j("⚠ Please enter a valid email");return}const t=_t();j("📧 Verifying email service…",3e3);try{await yx(t),await yo({to:e,subject:"Test Email — Livio Building Systems",message:`This is a test email from Livio Building Systems.
...
Best regards,
${qi()}`;h("lien-email-invid").value=e,h("lien-email-type").value=t,h("mo-lien-email").dataset.partialPaymentId=n||o.partialPaymentId||"",h("lien-email-title").textContent=t+" Lien Waiver",h("lien-email-to").value=o.vendorEmail||no(r),h("lien-email-subject").value=p+" — "+i+" / Inv #"+c,h("lien-email-body").value=y;const w=h("lien-email-status");w&&(w.innerHTML=Ln(_t())),h("mo-lien-email").classList.add("open")};Pi=function(e){const t=B();if(!t)return;const n=(t.vendors||[]).find(l=>l.id===e);if(!n)return;const o=(n.vendorEmail||no(n.vendor)||"").trim(),r="Subcontract Agreement — "+n.vendor+" / "+t.name,i=(n.milestones||[]).length?`
...
5. California law governs this agreement.`,d,m,11,"normal",b,17),f+=16,x(82);const ge=f+24;a.setDrawColor(120,120,120),a.setLineWidth(.8),a.line(d,ge,d+220,ge),a.line(c-u-220,ge,c-u,ge),S(9,"normal",b),a.text("Owner / General Contractor",d,ge+14),a.text("Subcontractor Signature",c-u-220,ge+14),S(11,"bold",y),a.text(se,d,ge+32),a.text(o.vendor||"Subcontractor",c-u-220,ge+32);const Vt=a.output("datauristring"),od=String(Vt).split(",")[1]||"",ea=String(o.vendor||"vendor-contract").replace(/[^a-z0-9-_]+/gi,"_"),xr=String(n.name||"project").replace(/[^a-z0-9-_]+/gi,"_");return{filename:`${ea}-${xr}-contract.pdf`,content:od,contentType:"application/pdf"}};async function Xx(e){return[Gl(e)]}Ql=async function(){const e=h("cemail-to").value.trim(),t=h("cemail-subject").value.trim(),n=h("cemail-body").value.trim(),o=h("cemail-vid").value.trim();if(!e){j("Recipient email is required");return}let r=[];try{o&&(r=await Xx(o))}catch(i){j("Could not attach contract PDF: "+i.message,"error");return}yo({to:e,subject:t,message:n,attachments:r}).then(function(){Ki(),j("Contract email sent with PDF attachment")}).catch(function(i){console.error("Email error:",i),j("Email send failed: "+i.message,"error")})};function Zx(e){const t=B();!t||!confirm("Delete this vendor contract?")||(t.vendors=(t.vendors||[]).filter(n=>n.id!==e),Z(),ne(),j("🗑 Deleted"))}function Jl(){const e=B();if(!e)return;const t=e.checklist||[];h("ct-chk").textContent=t.length,h("chk-total").textContent=t.length,h("chk-done").textContent=t.filter(r=>r.status==="done").length,h("chk-prog").textContent=t.filter(r=>r.status==="inprogress").length,h("chk-open").textContent=t.filter(r=>!r.status||r.status==="open").length;const n=h("chk-content");if(!n)return;if(!t.length){n.innerHTML='<div class="empty"><div class="empty-ic">☑</div>No checklist items yet. Click + Add Item to start.</div>';return}const o={};t.forEach(r=>{const i=r.category||"General";o[i]||(o[i]=[]),o[i].push(r)}),n.innerHTML=Object.entries(o).map(([r,i])=>{const a=i.filter(d=>d.status==="done").length,c=i.length?Math.round(a/i.length*100):0,l=i.map((d,u)=>{const m=u%2===0?"#fff":"var(--bg)",p=[["open","Open","var(--red)"],["inprogress","In Progress","var(--amber)"],["done","Done","var(--green)"]],g=(d.comments||[]).slice(-1)[0],b=(d.comments||[]).length;return`<tr style="background:${m}">
    ...
    </div>`}).join("")}function eh(e){const t=B();!t||!confirm("Delete this item?")||(t.checklist=(t.checklist||[]).filter(n=>n.id!==e),Z(),ne(),j("🗑 Deleted"))}function Kl(){const e=B();if(!e)return;const t=e.qaqcLog||[];h("ct-qaqc").textContent=t.length,h("qa-total").textContent=t.length,h("qa-ncr").textContent=t.filter(o=>o.type==="ncr"&&o.status!=="closed").length,h("qa-closed").textContent=t.filter(o=>o.status==="closed").length,h("qa-obs").textContent=t.filter(o=>o.type==="observation").length,t.forEach(o=>G(o.files||[]));const n=h("qa-content");if(n){if(!t.length){n.innerHTML='<div class="empty"><div class="empty-ic">🔍</div>No QA/QC entries yet. Click + Add QA/QC Entry to start.</div>';return}n.innerHTML=`<div class="panel">
    <table style="width:100%;border-collapse:collapse;table-layout:fixed">
    ...
    </div>`,h("mo").classList.add("open"),X="access-ctrl-"+e;const c=document.querySelector(".modal-foot");c&&(c.style.display="none")}let $i={};function gh(e,t,n){$i[e]=t;const o=n.closest('div[style*="border:1px solid"]');o&&(o.style.background=t==="full"?"var(--green-l)":t==="view"?"var(--amber-l)":"#fff")}function xh(e){hn.forEach(t=>{$i[t.id]=e,document.querySelectorAll(`input[name="pg_${t.id}"]`).forEach(n=>{n.checked=n.value===e})}),document.querySelectorAll('#modal-body div[style*="border:1px solid"]').forEach(t=>{t.style.background=e==="full"?"var(--green-l)":e==="view"?"var(--amber-l)":"#fff"})}function hh(e){const t={};hn.forEach(o=>{const r=document.querySelector(`input[name="pg_${o.id}"]:checked`);t[o.id]=r?r.value:"none"}),Object.assign(t,$i);const n=Xi();n[e]=t,uh(n),$i={},Gi(),he(),j("✓ Page access saved for "+e),Df()}function mo(){return Rt(),ji(L==null?void 0:L.roles)}function Pf(e){Rt(),L.roles=ji(e),Z()}function td(){const e=document.getElementById("nu-role");if(!e)return;const t=mo(),n=e.value||"Manager";e.innerHTML=t.map(o=>`<option value="${o}"${o===n?" selected":""}>${o}</option>`).join("")}function nd(){const e=mo();h("modal-title").textContent="Manage Roles",h("modal-body").innerHTML=`
    ...
    </div>`,h("mo").classList.add("open"),X="manage-roles";const t=document.querySelector(".modal-foot");t&&(t.style.display="none"),setTimeout(()=>{var n;return(n=document.getElementById("new-role-input"))==null?void 0:n.focus()},100)}function yh(){const e=document.getElementById("new-role-input"),t=((e==null?void 0:e.value)||"").trim();if(!t){j("⚠ Enter a role name");return}const n=mo();if(n.find(o=>o.toLowerCase()===t.toLowerCase())){j("⚠ Role already exists");return}n.push(t),Pf(n),td(),j('✓ Role "'+t+'" added'),nd()}function bh(e){if(!confirm('Remove role "'+e+'"?'))return;const n=Tn().find(r=>r.role===e);if(n){j('⚠ Cannot delete — role is assigned to user "'+n.username+'"');return}const o=mo().filter(r=>r!==e);Pf(o),td(),j('🗑 Role "'+e+'" removed'),nd()}function Tn(){return Rt(),Ai(L==null?void 0:L.users)}function jf(e){Rt(),L.users=Ai(e),Z()}function wh(){const e=document.getElementById("sidebar"),t=document.getElementById("sb-overlay");e.classList.toggle("mob-open"),t.classList.toggle("show")}function Af(){var e,t;(e=document.getElementById("sidebar"))==null||e.classList.remove("mob-open"),(t=document.getElementById("sb-overlay"))==null||t.classList.remove("show")}document.querySelectorAll(".sb-nav a").forEach(e=>e.addEventListener("click",()=>{window.innerWidth<=768&&Af()}));function $f(){const e=document.getElementById("mob-menu-btn");e&&(e.style.display=window.innerWidth<=768?"flex":"none")}window.addEventListener("resize",$f);$f();function Ch(){var a,c;const e=(((a=document.getElementById("login-user"))==null?void 0:a.value)||"").trim(),t=(((c=document.getElementById("login-pass"))==null?void 0:c.value)||"").trim(),n=document.getElementById("login-error"),o=document.querySelector('#login-screen button[onclick="doLogin()"]');if(!e||!t){n&&(n.style.display="",n.textContent="Please enter username and password.");return}o&&(o.textContent="Signing in…",o.disabled=!0);const i=Tn().find(l=>l.username===e&&l.password===t);if(i){sessionStorage.setItem(Xl,JSON.stringify({id:i.id,username:i.username,role:i.role}));const l=document.getElementById("login-screen");l&&(l.style.display="none"),n&&(n.style.display="none");const d=document.getElementById("sb-user-label");d&&(d.textContent=i.username+" ("+i.role+")"),document.body.classList.add("logged-in"),Gi(),Nl(),ho("projects"),setTimeout(()=>{Il(),zl(),Ll(),Tl(),Bl(),Ml(),Yi(),Vl(),ql(),Jl(),Kl(),Fl(),_l(),wo()},300)}else{o&&(o.textContent="Sign In →",o.disabled=!1),n&&(n.style.display="",n.textContent="Incorrect username or password.");const l=document.getElementById("login-pass");l&&(l.value="",l.focus())}}function Ot(){try{const e=sessionStorage.getItem(Xl);return e?JSON.parse(e):null}catch{return null}}function Sh(){sessionStorage.removeItem(Xl),document.body.classList.remove("logged-in");const e=document.getElementById("login-screen");e&&(e.style.display="flex");const t=document.getElementById("login-user"),n=document.getElementById("login-pass"),o=document.getElementById("sb-user-label");t&&(t.value="",setTimeout(()=>t.focus(),100)),n&&(n.value=""),o&&(o.textContent="");const r=document.getElementById("login-error");r&&(r.style.display="none")}function kh(){const e=document.getElementById("login-pass");e&&(e.type=e.type==="password"?"text":"password")}function Df(){const e=Ot();if(!e||e.role!=="Admin"){j("⚠ Only Admin can manage users");return}const n=Tn().map(r=>{const i=mo().map(a=>`<option value="${a}"${r.role===a?" selected":""}>${a}</option>`).join("");return`<tr style="background:#fff;border-bottom:1px solid var(--border)">
