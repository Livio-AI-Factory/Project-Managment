...
function generateVendorContract(vid){
  ...
  const proj_data=p;
  const signedOn=formatVendorContractSignedOn(v);
  ...
  const html='<!DOCTYPE html><html><head><meta charset="UTF-8"/>'
    ...
    +'<div class="sig-row">'
      +'<div class="sig-box">'
        +(signedOn?'<div style="height:38px;font-size:15px;color:#1A6BC4;font-style:italic;font-weight:600">'+LIVIO_COMPANY_NAME+'</div>':'<div style="height:50px"></div>')
        ...
        +'<div style="font-size:9px;color:#999;margin-top:3px">'+(signedOn?'Electronically signed on '+signedOn:'Date: ___________________')+'</div>'
      +'</div>'
      +'<div class="sig-box">'
        +(signedOn?'<div style="height:38px;font-size:15px;color:#1A6BC4;font-style:italic;font-weight:600">'+(v.signatureName||v.vendor||'Subcontractor')+'</div>':'<div style="height:50px"></div>')
        +'<div class="sig-label">Subcontractor Signature</div>'
        +'<div class="sig-name">'+v.vendor+'</div>'
        +'<div style="font-size:9px;color:#999;margin-top:3px">'+(signedOn?'Auto-signed on '+signedOn:'Date: ___________________')+'</div>'
      +'</div>'
    +'</div>'
...
function findVendorContractById(vid){
  const p=proj(); if(!p)return null;
  const v=(p.vendors||[]).find(x=>x.id===vid); if(!v)return null;
  const vdir=v.vdirId ? getVDirList().find(x=>x.id===v.vdirId) : null;
  return {p,v,vdir};
...
function getVendorContractSignedDate(v){
  const raw=v?.signedAt||v?.autoSignedAt||'';
  if(raw){
...
function formatVendorContractSignedOn(v){
  const signedDate=getVendorContractSignedDate(v);
  if(!signedDate) return '';
  return signedDate.toLocaleString('en-US',{
    year:'numeric',
    month:'short',
    day:'numeric',
    hour:'numeric',
...
function snapshotVendorContractSigning(v){
  const keys=['status','signedAt','autoSignedAt','signedBy','signatureName','signedMethod','sentForSignatureAt','sentForSignatureTo'];
  return keys.reduce(function(acc,key){
    acc[key]=Object.prototype.hasOwnProperty.call(v,key)?v[key]:undefined;
    return acc;
  },{});
}

function restoreVendorContractSigning(v,snapshot){
  if(!v||!snapshot)return;
  Object.keys(snapshot).forEach(function(key){
    if(typeof snapshot[key]==='undefined') delete v[key];
    else v[key]=snapshot[key];
  });
}

function prepareVendorContractAutoSignature(vid,to){
  const found=findVendorContractById(vid);
  if(!found) throw new Error('Vendor contract not found.');
  const now=new Date().toISOString();
  const snapshot=snapshotVendorContractSigning(found.v);
  found.v.status='signed';
  found.v.signedAt=now;
  found.v.autoSignedAt=now;
  found.v.signedBy=LIVIO_COMPANY_NAME;
  found.v.signatureName=found.v.vendor||'Subcontractor';
  found.v.signedMethod='auto-email';
  found.v.sentForSignatureAt=now;
  found.v.sentForSignatureTo=to||'';
  return {found,snapshot};
}

function signedVendorContractAttachmentName(attachment){
  const raw=String(attachment?.filename||'vendor-contract.pdf');
  if(/signed/i.test(raw)) return raw;
  return raw.replace(/\.pdf$/i,'')+'-signed.pdf';
}

function rememberSignedVendorContractAttachment(v,attachment){
  if(!v||!attachment||!attachment.content)return;
  const filename=signedVendorContractAttachmentName(attachment);
  const file={
    id:uid(),
    name:filename,
    filename,
    size:Math.ceil(String(attachment.content||'').length*0.75),
    type:attachment.contentType||'application/pdf',
    contentType:attachment.contentType||'application/pdf',
    data:'data:'+(attachment.contentType||'application/pdf')+';base64,'+attachment.content,
    at:v.signedAt||new Date().toISOString(),
    autoSignedContract:true,
    signedAt:v.signedAt||''
  };
  if(typeof FA!=='undefined') FA[file.id]=file;
  v.files=[file,...(v.files||[]).filter(function(existing){
    return !(existing.autoSignedContract&&existing.signedAt===file.signedAt);
  })];
}
